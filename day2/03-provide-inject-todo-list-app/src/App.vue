<template>
  <div>
    <app-header />
    <div class="container">
      <div class="row mt-5">
        <div class="col-4 offset-4 card p-0">
          <div class="card-header">TodoList</div>
          <div class="card-body">
            <AddSection @add-item-event="addTodo" />
            <ListSection />
          </div>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
import AddSection from "@/components/AddSection";
import ListSection from "@/components/ListSection";
export default {
  components: {
    AddSection,
    ListSection
  },
  data() {
    return {
      myText: "bu değer app.vue dosyası tarafından gönderilmiştir..",
      provideData: {
        todoList: [
          { id: 1, text: "Todo1", completed: false },
          { id: 2, text: "Todo2", completed: false },
          { id: 3, text: "Todo3", completed: false },
          { id: 4, text: "Todo4", completed: false }
        ]
      }
    };
  },
  provide() {
    return {
      // itemList: ["Elma", "Kiraz", "Karpuz"]
      provideData: this.provideData,
      deleteTodo: this.deleteTodo
    };
  },
  methods: {
    addTodo(event) {
      this.provideData.todoList.push(event);
    },
    deleteTodo(todo) {
      this.provideData.todoList = this.provideData.todoList.filter(t => t !== todo);
    }
  },
  mounted() {
    setTimeout(() => {
      this.provideData.todoList.push({
        id: 5,
        text: "Todo5",
        completed: false
      });
    }, 1000);
  }
};
</script>
